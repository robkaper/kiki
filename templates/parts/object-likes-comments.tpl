<ul class="actions">
  <li><button{if !$kiki.user.id} disabled{/if} id="object-{{$object_id}}-likes-button" class="anchor objectButton{if $likes.active} active{/if}" data-object-id="{{$object_id}}" data-action="likes"><i class="fa-solid fa-medal"></i> Likes</button><span id="object-{{$object_id}}-likes" class="smaller">{{$likes.count}}</span></li>
